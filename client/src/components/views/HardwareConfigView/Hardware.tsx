...
export class Robot {
  ...
  public toString(): string {
    ...
    if (this.controlHubs.length > 0 || this.expansionHubs.length > 0) {
      lines.push(`    <LynxUsbDevice name="Control Hub Portal" parentModuleAddress="173" serialNumber="(embedded)">`);
  ...
  public fromXml(xmlString: string): void {
    ...
    const getAttr = (el: Element, name: string, def: string | null = null): string | null => el.getAttribute(name) ?? def;

    const createDeviceInstance = (element: Element, category: 'Motor' | 'Servo' | 'Analog' | 'Digital' | 'I2c'): Device => {
        let device: Device;
        const tagName = element.tagName;

        switch (category) {
            case 'Motor':   device = new Motor();   break;
            case 'Servo':   device = new Servo();   break;
            case 'Analog':  device = new Analog();  break;
            case 'Digital': device = new Digital(); break;
            case 'I2c':     device = new I2c();     break;
            default: throw new Error(`Unknown device category: ${category}`);
        }
        ...
        if (childNode.tagName === "LynxUsbDevice") {
            ...
            for (const moduleNode of Array.from(lynxUsbDeviceElement.children)) {
                if (moduleNode.tagName === "LynxModule") {
                    ...
                    let currentHub: Hub;
                    if ((hubNameAttr && hubNameAttr.toLowerCase().includes("control hub")) || hubPort === 173) {
                    ...
                    for (const deviceElement of Array.from(lynxModuleElement.children)) {
                        ...
                        } else if (Object.values(analogType).includes(tagName as string)) {
                            deviceInstance = createDeviceInstance(deviceElement, 'Analog');
                            currentHub.analogInputDevices.push(deviceInstance as Analog);
                        } else if (Object.values(digitalType).includes(tagName as string)) {
            ...
            ethDevice.serialNumber = getAttr(ethDeviceElement, "serialNumber") || "";
  ...
  public renderAsGui(configChangeCallback: () => void, keyPrefix: string): JSX.Element {
    return (
      <div key={keyPrefix} style={{ padding: '10px', fontFamily: 'Arial, sans-serif' }}>
        <h3>Robot Configuration</h3>
        <button style={addButtonStyle} onClick={() => { this.controlHubs.push(new ControlHub()); configChangeCallback(); }}>+</button> Control Hub
        <button style={addButtonStyle} onClick={() => { this.expansionHubs.push(new ExpansionHub()); configChangeCallback(); }}>+</button> Expansion Hub
        <hr />
        {this.controlHubs.map((hub, index) => hub.renderAsGui(
        ...
        {this.expansionHubs.map((hub, index) => hub.renderAsGui(
            configChangeCallback, 
            `${keyPrefix}-eh-${index}`,
            () => {
        ...
        <h4>Other Devices</h4>
        <button style={addButtonStyle} onClick={() => { this.devices.push(new EthernetDevice()); configChangeCallback(); }}>+</button> Ethernet Device
        <button style={addButtonStyle} onClick={() => { this.devices.push(new Webcam()); configChangeCallback(); }}>+</button> Webcam
        {this.devices.map((device, index) => device.renderAsGui(
            configChangeCallback, 
            `${keyPrefix}-od-${index}`,
            () => {
...
export abstract class Device {
  ...
  public abstract renderAsGui(configChangeCallback: () => void, keyPrefix: string, onDelete?: () => void): JSX.Element;
}

const deviceContainerStyle: React.CSSProperties = { marginLeft: '20px', border: '1px solid #ccc', padding: '10px', margin: '10px', backgroundColor: '#f9f9f9', position: 'relative', borderRadius: '8px' };
const inputStyle: React.CSSProperties = { marginRight: '10px', padding: '4px', border: '1px solid #ddd', borderRadius: '4px' };
const labelStyle: React.CSSProperties = { marginRight: '5px', fontWeight: 'bold' };
...
const hubDeleteButtonStyle: React.CSSProperties = { ...deleteButtonStyle, position: 'absolute', top: '15px', right: '15px' };
...
export abstract class Hub extends Device {
  ...
  public override toString(): string {
    const lines: string[] = [];
    lines.push(`<${this.key} name="${this.name}" port="${isNaN(this.port) ? 0 : this.port}">`);
  ...
  public renderAsGui(configChangeCallback: () => void, keyPrefix: string, onDelete?: () => void): JSX.Element {
    return (
      <div key={keyPrefix} style={{ border: '2px solid #007bff', padding: '15px', margin: '15px', borderRadius: '8px', backgroundColor: '#e7f3ff', position: 'relative' }}>
        <h4>
          <label style={labelStyle}>Hub Name: </label>
          <input style={inputStyle} type="text" value={this.name} onChange={e => { this.name = e.target.value; configChangeCallback(); }} />
          <label style={labelStyle}>Port: </label>
          <input style={inputStyle} type="number" value={isNaN(this.port) ? "" : this.port} onChange={e => { this.port = parseInt(e.target.value, 10); configChangeCallback(); }} />
           ({this.type})
           {onDelete && <button style={hubDeleteButtonStyle} onClick={onDelete}>X</button>}
        </h4>

        <div>
            <h5>Motors <button style={addButtonStyle} onClick={() => { this.motors.push(new Motor()); configChangeCallback(); }}>+</button></h5>
            {this.motors.map((motor, index) => motor.renderAsGui(configChangeCallback, `${keyPrefix}-motor-${index}`, () => { this.motors.splice(index, 1); configChangeCallback(); }))}
        </div>
        <div>
            <h5>Servos <button style={addButtonStyle} onClick={() => { this.servos.push(new Servo()); configChangeCallback(); }}>+</button></h5>
            {this.servos.map((servo, index) => servo.renderAsGui(configChangeCallback, `${keyPrefix}-servo-${index}`, () => { this.servos.splice(index, 1); configChangeCallback(); }))}
        </div>
        <div>
            <h5>I2C Devices <button style={addButtonStyle} onClick={() => { this.i2cDevices.push(new I2c()); configChangeCallback(); }}>+</button></h5>
            {this.i2cDevices.map((i2c, index) => i2c.renderAsGui(configChangeCallback, `${keyPrefix}-i2c-${index}`, () => { this.i2cDevices.splice(index, 1); configChangeCallback(); }))}
        </div>
        <div>
            <h5>Analog Inputs <button style={addButtonStyle} onClick={() => { this.analogInputDevices.push(new Analog()); configChangeCallback(); }}>+</button></h5>
            {this.analogInputDevices.map((analog, index) => analog.renderAsGui(configChangeCallback, `${keyPrefix}-analog-${index}`, () => { this.analogInputDevices.splice(index, 1); configChangeCallback(); }))}
        </div>
        <div>
            <h5>Digital Devices <button style={addButtonStyle} onClick={() => { this.digitalDevices.push(new Digital()); configChangeCallback(); }}>+</button></h5>
            {this.digitalDevices.map((digital, index) => digital.renderAsGui(configChangeCallback, `${keyPrefix}-digital-${index}`, () => { this.digitalDevices.splice(index, 1); configChangeCallback(); }))}
        </div>
      </div>
    );
  }
  ...
  public renderAsGui(configChangeCallback: () => void, keyPrefix: string, onDelete?: () => void): JSX.Element {
    return (
      <div key={keyPrefix} style={deviceContainerStyle}>
        {onDelete && <button style={{...deleteButtonStyle, position: 'absolute', top: '5px', right: '5px'}} onClick={onDelete}>X</button>}
        <div>
        ...
        <div>
          ...
          <input style={inputStyle} type="text" value={this.serialNumber} onChange={e => { this.serialNumber = e.target.value; configChangeCallback(); }} />
        </div>
        <div>
          <label style={labelStyle}>IP Address: </label>
          <input style={inputStyle} type="text" value={this.ipAddress} onChange={e => { this.ipAddress = e.target.value; configChangeCallback(); }} />
        </div>
      </div>
    );
  }
...
export class Webcam extends Device {
  ...
  public override toString(): string { // Webcam doesn't have a port attribute in its XML
        ...
        <div key={keyPrefix} style={deviceContainerStyle}>
            ...
            <div>
                ...
                <input 
                    ...
                    type="text" 
                    value={device.name} 
                    onChange={e => { device.name = e.target.value; configChangeCallback(); }} 
                />
            </div>
            <div>
                ...
                <select 
                    style={selectStyle} 
                    value={device.type} 
                    onChange={e => { device.type = e.target.value; device.key = e.target.value; configChangeCallback(); }}
                >
                    {Object.entries(typeObject).map(([displayName, xmlName]) => (
                        <option key={xmlName} value={xmlName}>{displayName}</option>
                    ))}
                </select>
            </div>
            <div>
                ...
                <input 
                    ...
                    onChange={e => { device.port = parseInt(e.target.value, 10); configChangeCallback(); }} 
                />
            </div>
            {additionalFields}
        </div>
...
export class Motor extends Device {
    ...
    public override toString(): string { return `<${this.key} name="${this.name}" port="${isNaN(this.port) ? 0 : this.port}" />`; }
    public renderAsGui(configChangeCallback: () => void, keyPrefix: string, onDelete?: () => void): JSX.Element {
...
export class I2c extends Device {
    ...
    public override toString(): string { return `<${this.key} name="${this.name}" port="${isNaN(this.port) ? 0 : this.port}" bus="${isNaN(this.bus) ? 0 : this.bus}" />`; }
    public renderAsGui(configChangeCallback: () => void, keyPrefix: string, onDelete?: () => void): JSX.Element {
        const busField = (
            <div>
                ...
                <input 
                    ...
                    onChange={e => { this.bus = parseInt(e.target.value, 10); configChangeCallback(); }} 
                />
            </div>
        );
